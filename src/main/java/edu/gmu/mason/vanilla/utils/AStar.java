 ...
 * A class that uses A* algorithm to find optimal paths between graph nodes
 * 
 */
public class AStar {
    ...
    public ArrayList<GeomPlanarGraphDirectedEdge> astarPath(Node start, Node goal) {
        ...
        while (openSet.size() > 0) { // while there are reachable nodes to investigate
            ...
            for (Object o : des.getEdges().toArray()) {
                ...
                double tentativeCost = x.gx + weight(l);
        ...
        // we couldn't find the shortest path
        return result;
    }
     ...
     */
    public static double length(GeomPlanarGraphDirectedEdge e) {
     ...
     */
    public static double weight(GeomPlanarGraphDirectedEdge e) {
        double weight = 1;
        Edge edge = e.getEdge();
        if (edge instanceof GeomPlanarGraphEdge && ((GeomPlanarGraphEdge) edge).hasAttribute("weight")) {
            weight = ((GeomPlanarGraphEdge) e.getEdge()).getDoubleAttribute("weight");
        }
        return length(e) * weight;
    }

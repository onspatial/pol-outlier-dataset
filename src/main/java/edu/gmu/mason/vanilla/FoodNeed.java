...
public class FoodNeed implements Need, java.io.Serializable {
		...
		if (isSatisfied() == false && currentMode != PersonMode.Transport
			...
			if (currentMode == PersonMode.AtHome) {
				...
				if (preConditionToEatHome
					...
					Restaurant restaurant = this.findRestaurant();
				...
				if (pub.getHourlyCost() <= agent.getFinancialSafetyNeed().getRemainingFoodBudgetForToday()) {
					...
					// double budget =
					// agent.getFinancialSafetyNeed().getRemainingFoodBudgetForToday();
	...
	private Restaurant findRestaurant() {
		...
		if (agent.getFavoriteRestaurants(budget) != null && agent.getFavoriteRestaurants(budget).size() > 0) {
			restaurant = agent.getFavoriteRestaurants(budget).get(0);
		} else {
			restaurant = agent.getCurrentUnit().getNearestRestaurant(budget);
			if (restaurant != null
					&& agent.getFavoriteRestaurants().size() < agent
							.getModel().params.maximumNumberOfFavoriteRestaurants) {
				agent.addFavoriteRestaurant(restaurant);
			}

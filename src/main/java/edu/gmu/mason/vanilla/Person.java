...
public class Person implements Steppable, java.io.Serializable {
	...
	public Person(WorldModel model, long id, Family family) {
		...
		this.favoritePubs = new ArrayList<Pub>();
	...
	public DailyPlan planForSpecificDay(int dayDifferenceFromToday) {
		...
		if (this.financialSafetyNeed.isEmployed()
			...
			if (model.isAddNoiseToSchedules()) {
				// nextInt(n) Returns an integer drawn uniformly from 0 to n-1
				int noiseAsTick = model.random.nextInt(9) - 4;
				commuteLenghtAsTicks += noiseAsTick;
			}
	...
	public List<Restaurant> getFavoriteRestaurants(double budget) {

		if (favoriteRestaurants != null) {
			List<Restaurant> affordableRestaurants = new ArrayList<Restaurant>();
			for (Restaurant restaurant : favoriteRestaurants) {
				if (restaurant.getFoodCost() <= budget) {
